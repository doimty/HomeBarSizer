...
check("设置页用 FrontBoard respring", "SBSRelaunchAction" in PREFS and "FBSSystemService" in PREFS)
check("respring 回退走 jbroot", "jbroot" in PREFS and "posix_spawn" in PREFS)
check("respring 不写死 /var/jb", "/var/jb" not in PREFS)
...
check("Version 1.3.3", re.search(r"^Version: 1.3.3$", CONTROL, re.M) is not None)
check("Icon 用带版本的 HTTPS 地址", "icons/com.imkpatil.homebarsizer-1.3.2.png" in CONTROL)
